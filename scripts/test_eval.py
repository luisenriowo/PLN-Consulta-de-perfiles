...
PRED = [
    ...
    TimelineEntry(fecha=date(2023, 4, 13),
...
def main() -> None:
    ...
    assert math.isclose(metrics.rouge_1(par_p1[0].resumen, par_p1[1].resumen), 1.0), "P1 idéntico"
    ...
    print(f"  {{'tasa': {al['tasa']:.3f}, 'no_respaldadas': {al['no_respaldadas']}, 'total': {al['total']}}}")
    assert al["no_respaldadas"] == 1 and math.isclose(al["tasa"], 1 / 3, rel_tol=1e-9), al
    print("  OK: la agregación (fracción no respaldada) computa bien.")

    print("\n== Tasa de alucinación: JUEZ NLI en los casos claros ==")
    al_nli = metrics.tasa_alucinacion(PRED, FUENTES_TEXTO)  # verificador NLI por defecto
    veredictos = {e.fuentes[0]: ok for e, ok in al_nli["detalle"]}
    print(f"  veredictos NLI (respaldado?): {veredictos}")
    print(f"  tasa NLI: {al_nli['tasa']:.3f}  no_respaldadas={al_nli['no_respaldadas']}")
    assert veredictos["d1"] is True, "el match perfecto debe quedar RESPALDADO"
    assert veredictos["d3"] is False, "la alucinación (absuelto vs condena) debe quedar NO respaldada"
    print("  OK: el juez NLI clasifica bien P1 (respaldado) y P3 (alucinado).")
    print("  ⚠ Para el número titular: validar el juez con eval.nli.validar_juez "
