...
def test_multi_fuente_decisiva() -> None:
    base = dict(
        cluster_id="e", fecha_normalizada=date(2021, 1, 1),
...
def test_dedup_cross_fuente() -> None:
    txt = ("Titular del evento. Cuerpo de la nota con longitud suficiente para "
           "superar el mínimo de caracteres del preprocess y no ser descartada.")
    d1 = Documento(doc_id="andina:1", fuente="andina.pe", url="u1",
                   fecha_pub=date(2021, 1, 1), texto=txt)
    d2 = Documento(doc_id="gdelt:https://otromedio.pe/n", fuente="otromedio.pe",
                   url="u2", fecha_pub=date(2021, 1, 2), texto=txt)
